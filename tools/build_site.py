...
{alternates}
<link rel="icon" href="/assets/favicon.svg?v=20260909-beige-brown-2" type="image/svg+xml">
...
<link rel="canonical" href="{canonical}">
{alternates}
<link rel="icon" href="{prefix}assets/favicon.svg?v=20260909-beige-brown-2" type="image/svg+xml">
...
def build_redirects(news_index: list[dict]) -> str:
    # Legacy ".html" URLs are served as HTTP 301s by Netlify instead of HTML
    # stub files. Stub files sit at the clean path (e.g. /researcher.html is
    # also served for /researcher), which collides with the directory page and,
    # combined with Netlify's trailing-slash normalisation, loops /researcher/.
    # A _redirects entry has no such collision.
    lines = [
        "# Legacy .html bookmarks -> clean directory URLs (HTTP 301).",
        "# Generated by tools/build_site.py — do not edit by hand.",
        "",
    ]
    role_pages = [p for p in PAGES if p not in ("index.html", "release.html")]
    for locale in LOCALES:
        pre = "" if locale == "ja" else f"/{locale}"
        for page in role_pages:
            slug = page.removesuffix(".html")
            lines.append(f"{pre}/{page}\t{pre}/{slug}/\t301")
        for article in news_index:
            s = article["slug"]
            lines.append(f"{pre}/news/{s}.html\t{pre}/news/{s}/\t301")
        # release.html carried an optional ?slug=; preserve it, else send to the list.
        lines.append(f"{pre}/release.html\tslug=:slug\t{pre}/news/:slug/\t301")
        lines.append(f"{pre}/release.html\t{pre}/news/\t301")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    manifest = {"name":"Akira Kusama Portfolio","short_name":"Akira Kusama","start_url":"/","display":"standalone","background_color":"#ECEBE4","theme_color":"#2C3A86","icons":[{"src":"/assets/favicon.svg?v=20260909-beige-brown-2","sizes":"any","type":"image/svg+xml","purpose":"any"}]}
